      ...
      <el-table-column label="操作" width="270">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="handleAgreeReturn(scope.$index, scope.row)"
            >同意退货</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleOk(index, row) {
      ...
      if (
        row.whetherShip == 1 &&
        row.whetherReturn == 3 &&   //确认收货
        row.payStatus == 1
      ) {
        this.$axios
          .put("http://localhost:8181/shopcar/ordercomplete/" + row.id)
          .then((result) => {
            let ins = Toast("订单完结!!");
            setTimeout(() => {
              ins.close;
    ...
    handleAgreeReturn(index, row) {
      if (row.whetherReturn == 1) {
        console.log(index, row);
        this.$axios
          .put("http://localhost:8181/shopcar/agreereturn/" + row.id)
          .then((result) => {
            let ins = Toast("退货成功!!");
            setTimeout(() => {
              ins.close;
