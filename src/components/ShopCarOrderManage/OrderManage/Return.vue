<template>
  <div>
    ...
    >
      ...
      <el-table-column label="库存返回">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag size="medium">{{
              whetherReturnStock(scope.row.returnStock)
            }}</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="270">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="handleReturnStock(scope.$index, scope.row)"
            >退货完成（商品返回库存）</el-button
          >
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getphoneIdAndquantity(row) {
      let isSuccess = false;
      console.log(row);
      await this.$axios
        .get("http://localhost:8181/shopcar/findOrderDetail")
        .then((result) => {
          //  console.log(result);
          isSuccess = true;
          for (let i = 0; i < result.data.length; i++) {
            if (result.data[i].orderId == row.id) {
              this.tableDataforreturnstock.unshift(result.data[i]);
    ...
    returnStock(row) {
      // if (row.returnStock == 0) {
      console.log("111");
      console.log(this.tableDataforreturnstock);
      for (let i = 0; i < this.tableDataforreturnstock.length; i++) {
        let shopCarReturnStockForm = {
          quantity: this.tableDataforreturnstock[i].quantity,
          phoneId: this.tableDataforreturnstock[i].phoneId,
          // orderId11:  this.tableDataforreturnstock[i].orderId
        };
        console.log(this.shopCarReturnStockForm);
        console.log("222");
        // console.log(this.shopCarReturnStockForm)
        this.$axios
          .post(
            "http://localhost:8181/shopcar/returnStock",
            shopCarReturnStockForm
          )
          .then((result) => {
            console.log("333");
            console.log(result);
            let ins = Toast("库存返回成功!!");
            setTimeout(() => {
              ins.close;
              this.$router.go(0);
            }, 1000);
      ...
      // }
      // else {
      //   let ins = Toast("库存已经返回了！！无需再返回");
      //   setTimeout(() => {
      //     ins.close;
      //   }, 1000);
      // }
    },
    handleReturnStock(index, row) {
      if (row.returnStock == 0) {
        this.getphoneIdAndquantity(row).then((result) => {
          console.log(result);
          if (result) {
            this.returnStock();
          }
