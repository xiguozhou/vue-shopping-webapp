<template>
  <div>
    ...
    >
      ...
      <el-table-column label="申请退货" width="100">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag size="medium">{{
      ...
      <el-table-column label="操作" width="270">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="handleReceipt(scope.$index, scope.row)"
            >确认收货</el-button
          >
          <el-button
            size="mini"
...
export default {
  ...
  methods: {
    ...
    handleReturn(index, row) {
      ...
      if (row.whetherReturn == 0 && row.whetherShip == 1) {
        this.$axios
          .put("http://localhost:8181/shopcar/returngoods/" + row.id)
          .then((result) => {
            let ins = Toast("申请退货成功!!");
            setTimeout(() => {
              ins.close;
              this.$router.go(0);
            }, 1000);
        ...
        let ins = Toast("此情况不能申请退货!!");
        setTimeout(() => {
          ins.close;
        }, 1000);
      }
    },
    handleReceipt(index, row) {
      console.log(index, row);
      if (row.whetherShip == 1 && row.whetherReturn == 0) {
        this.$axios
          .put("http://localhost:8181/shopcar/cofirmreceipt/" + row.id)
          .then((result) => {
            let ins = Toast("确认收货成功!!");
            setTimeout(() => {
              ins.close;
              this.$router.go(0);
            }, 1000);
